...
def run():
    ...
    print(str(sr) + "hz, " + str(af) + ' samples/frame, ' +  "{:.2}".format(pa) + " rms max")
    ...
    with tqdm(total=vl) as pbar:
        for (i, block) in enumerate(genblocks(input, af)):
            ...
            base = (ImageOps.colorize(
                Image.fromarray(np.uint8(doubled), 'L')
                .resize((fps*scale, fps*scale), resample=Image.NEAREST),
                black = "#f2dce9",
                white = "#590f3c")
            ).convert("RGBA")
